class User < ActiveRecord::Base
  ...
  has_many :messages, -> { select("`messages`.*, `messages_users`.`read` AS `read`, `messages_users`.`recipient_deleted` as `deleted`") }, :through => :messages_user
  ...
  def self.system_notify_all(subject,message)
    m=messages.create(:subject=>'[Systemweit] '+subject,:body=>message)
